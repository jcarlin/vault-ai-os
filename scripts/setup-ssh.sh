...
print_summary() {
  ...
  echo "  SSH port         : 22"
  echo "  Test locally     : ssh $(whoami)@localhost"
  echo "  Test from LAN    : ssh <username>@${ip}"
  echo ""
  echo "  To check status  : sudo systemctl status ssh  (or sshd)"
  echo "  To view logs     : sudo journalctl -eu ssh    (or sshd)"
  echo ""
  info "============================================================"
  info "NEXT: To access from outside the home network, see the"
  info "remote access guide in this repo (REMOTE-ACCESS.md) or"
  info "install Tailscale:  curl -fsSL https://tailscale.com/install.sh | sh"
  info "============================================================"
}
