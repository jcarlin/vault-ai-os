...
source "virtualbox-iso" "ubuntu-2204" {
  ...
  # Path is relative to template location (../http from ubuntu-22.04/)
  http_directory = "../http"
  ...
  # Increased timeouts to 60m and handshake attempts to 420 (community-proven for Ubuntu 22.04)
  ssh_username           = var.ssh_username
  ssh_password           = var.ssh_password
  ssh_timeout            = "60m"  # Increased to 60m for maximum reliability
  ssh_handshake_attempts = 420    # Increased to 420 for Ubuntu 22.04 (works around Packer timeout bug)
  ssh_pty                = true
...
# Local VirtualBox build (existing, no GPU)
build {
  name    = "local-dev"
  ...
  post-processor "manifest" {
    output = "packer-manifest-local.json"
    strip_path = true
  }
}

# GCP GPU Build removed - use sources-gcp.pkr.hcl for GCP builds
